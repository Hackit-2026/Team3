...
# ホワイトリスト（モザイク除外する登録者）: [{name, embedding(list[float]), thumb(bytes)}]
if "whitelist" not in st.session_state:
    st.session_state.whitelist = None  # 遅延ロード（初回にディスクから読む）
if "last_exempt_report" not in st.session_state:
    st.session_state.last_exempt_report = None
...
# ホワイトリスト照合用：顔の特徴量（埋め込み）を出すImageEmbedder（完全オフライン）
...
def load_face_embedder():
    """ホワイトリスト照合用のImageEmbedderを初期化（完全オフライン）。"""
    try:
        ...
        st.error(f"顔照合エンジン(ImageEmbedder)の初期化に失敗しました: {e}")
        return None
...
# -------------------------------------------------------------------
# 4.5. 【追加機能】ホワイトリスト（登録者はモザイク除外）
#      既存の検出・マスク処理には手を加えず、後段のフィルタとして動作する。
# -------------------------------------------------------------------
MAX_WHITELIST = 5
WHITELIST_STORE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "whitelist_store.pkl"
)


def load_whitelist():
    """ディスクからホワイトリストを読み込む（無ければ空リスト）。完全ローカル。"""
    import pickle
...
def save_whitelist(whitelist):
    ...
    try:
        with open(WHITELIST_STORE_PATH, "wb") as f:
            pickle.dump(whitelist[:MAX_WHITELIST], f)
    except Exception as e:
        st.warning(f"ホワイトリストの保存に失敗しました: {e}")


def _wl_detect_face_boxes(image: Image.Image, grids: list):
    """照合用に顔の矩形だけを検出して返す [(x,y,w,h), ...]。

    既存 get_mask_boxes_locally と同じ detector・同じ妥当性しきい値を用いるため、
    マスク対象として検出される顔と同じ集合になる（ランドマークは使わず軽量）。
    """
    if detector is None:
        return []
    ...
    return py_nms(raw_faces, iou_threshold=0.35)


def compute_face_embedding(image: Image.Image, box):
    """顔矩形を切り出してImageEmbedderで特徴量(np.float32, L2正規化済み)を返す。失敗時 None。"""
    embedder = load_face_embedder()
    if embedder is None:
        return None
    x, y, w, h = box
    orig_w, orig_h = image.size
...
def register_face_from_image(name: str, pil_image: Image.Image, grids: list):
    """登録用写真から最も大きい顔を1つ検出・特徴量化し、エントリを返す。顔が無ければ None。"""
    if pil_image.mode != "RGB":
        pil_image = pil_image.convert("RGB")
    boxes = _wl_detect_face_boxes(pil_image, grids if grids else [1])
    if not boxes:
        return None
    # 最も大きい顔を本人とみなす
    ...
    return {"name": name.strip() or "登録者", "embedding": emb.tolist(), "thumb": buf.getvalue()}


def compute_whitelist_exemptions(image: Image.Image, grids: list, whitelist: list, threshold: float):
    """検出した各顔をホワイトリストと照合し、除外すべき顔矩形と照合レポートを返す。

    戻り値: (exempt_boxes[list[(x,y,w,h)]], report[list[dict]])
    各顔について、最も類似度が高い登録者を求め、しきい値以上なら除外する。
    """
    exempt_boxes = []
    report = []
    if not whitelist:
        return exempt_boxes, report

    wl_embs = [np.asarray(e["embedding"], dtype=np.float32) for e in whitelist]
    face_boxes = _wl_detect_face_boxes(image, grids)
    ...
    for (fx, fy, fw, fh) in face_boxes:
        ...
        exempt = best_sim >= threshold
        if exempt:
            exempt_boxes.append((fx, fy, fw, fh))
        report.append({
            "box": (fx, fy, fw, fh),
            "best_name": whitelist[best_idx]["name"],
            "best_sim": best_sim,
            "exempt": exempt,
        })
    return exempt_boxes, report
...
def _box_center_inside(item_box, regions, tol_ratio=0.15):
    """item_box の中心が、いずれかの除外顔矩形(少し拡張)の内側にあれば True。"""
...
def filter_out_whitelisted(items: list, exempt_boxes: list):
    """除外顔矩形に中心が含まれるマスク項目を取り除く（既存itemsを加工しない新規リストを返す）。"""
    if not exempt_boxes:
        return items
...
# -------------------------------------------------------------------
# 6.5. 【追加機能】ホワイトリスト登録UI（最大5名・完全ローカル）
# -------------------------------------------------------------------
if st.session_state.whitelist is None:
    st.session_state.whitelist = load_whitelist()

with st.expander(f"👤 ホワイトリスト（登録者はモザイクしない）— 最大 {MAX_WHITELIST} 名", expanded=False):
    st.caption("撮影・加工の前に持ち主の顔を登録すると、その人だけモザイクから除外されます。データはこのPC内にのみ保存されます。")

    wl = st.session_state.whitelist

    col_a, col_b = st.columns(2)
    with col_a:
        whitelist_enabled = st.checkbox("ホワイトリストを有効にする", value=True, key="wl_enabled")
    with col_b:
        whitelist_threshold = st.slider(
            "本人と判定する類似度のしきい値", 0.50, 0.95, 0.70, 0.01, key="wl_threshold",
            help="高いほど厳しく（他人を誤って除外しにくい）、低いほど本人を取りこぼしにくい。"
                 "目安：同一人物は概ね0.75以上、別人は0.5未満に出ます。",
        )

    st.markdown("---")
    if wl:
        st.markdown("**登録済み**")
        cols = st.columns(MAX_WHITELIST)
        for i, entry in enumerate(wl):
            with cols[i]:
                st.image(entry["thumb"], caption=entry["name"], use_column_width=True)
                if st.button("🗑 削除", key=f"wl_del_{i}", use_container_width=True):
                    wl.pop(i)
                    save_whitelist(wl)
                    st.rerun()
    else:
        st.info("まだ誰も登録されていません。")

    st.markdown("---")
    if len(wl) >= MAX_WHITELIST:
        st.warning(f"登録は最大 {MAX_WHITELIST} 名までです。追加するには既存の登録を削除してください。")
    else:
        st.markdown(f"**新規登録（あと {MAX_WHITELIST - len(wl)} 名）**")
        new_name = st.text_input("名前（任意）", key="wl_new_name", placeholder="例：本人")
        src = st.radio("顔写真の取得方法", ["ファイルを選ぶ", "カメラで撮影"], horizontal=True, key="wl_src")

        reg_img = None
        if src == "ファイルを選ぶ":
            wl_file = st.file_uploader(
                "顔がはっきり写った写真", type=["jpg", "jpeg", "png", "webp", "bmp"], key="wl_uploader"
            )
            if wl_file is not None:
                try:
                    reg_img = Image.open(wl_file)
                except Exception as e:
                    st.error(f"画像の読み込みに失敗しました: {e}")
        else:
            wl_cam = st.camera_input("正面の顔を撮影してください", key="wl_camera")
            if wl_cam is not None:
                try:
                    reg_img = Image.open(wl_cam)
                except Exception as e:
                    st.error(f"カメラ画像の読み込みに失敗しました: {e}")

        if st.button("＋ この顔を登録する", key="wl_add", type="primary", disabled=(reg_img is None)):
            with st.spinner("顔を解析して登録中..."):
                entry = register_face_from_image(new_name, reg_img, grids)
            if entry is None:
                st.error("顔を検出できませんでした。正面ではっきり写った写真でお試しください。")
            else:
                wl.append(entry)
                save_whitelist(wl)
                st.success(f"「{entry['name']}」を登録しました。")
                st.rerun()

if whitelist_enabled and st.session_state.whitelist:
    st.info(f"🟢 ホワイトリスト有効：{len(st.session_state.whitelist)} 名を照合し、一致した顔はモザイクしません。")
...
if uploaded_file is not None:
    ...
    if st.session_state.uploaded_file_data is not None:
        ...
        if st.button("🚀 AIモザイク解析を開始", type="primary"):
            if not mask_targets:
                ...
                with st.spinner("AIが顔を精密検出中..."):
                    try:
                        # 既存の検出処理（変更なし）
                        detected_items = get_mask_boxes_locally(
                            input_image,
                            mask_targets,
                            grids=grids,
                        )
                        ...
                        # 【追加】ホワイトリスト照合：登録者に一致した顔をマスク対象から除外
                        st.session_state.last_exempt_report = None
                        if whitelist_enabled and st.session_state.whitelist:
                            exempt_boxes, report = compute_whitelist_exemptions(
                                input_image, grids, st.session_state.whitelist, whitelist_threshold
                            )
                            detected_items = filter_out_whitelisted(detected_items, exempt_boxes)
                            st.session_state.last_exempt_report = report

                        st.session_state.boxes = detected_items
                        st.session_state.confirmed = False
                        if not st.session_state.boxes:
                            if detected_count > 0:
                                st.info("検出された顔はすべてホワイトリストの登録者と一致したため、モザイク対象はありません。")
                            else:
                                st.info("顔が検出されませんでした。「限界突破」モードに変更してお試しください。")
                        else:
                            exempted = detected_count - len(st.session_state.boxes)
                            msg = f"解析成功！ {len(st.session_state.boxes)} 箇所をモザイク対象として検出しました。"
                            if exempted > 0:
                                msg += f"（ホワイトリスト一致で {exempted} 箇所を除外）"
                            st.success(msg)
                    except Exception as e:
        ...
        if st.session_state.boxes is not None:
            ...
            # 【追加】ホワイトリスト照合レポート（しきい値の調整に利用）
            report = st.session_state.last_exempt_report
            if report:
